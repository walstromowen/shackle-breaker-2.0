...
export const BIOME_DEFINITIONS = { 
  ...
  DESERT: { 
    ...
    shapeElevation: (noise) => { 
      if (noise < -0.20) { return noise; } 
      else if (noise < 0.50) { return 0.05 + (noise * 0.1); } 
      else { return Math.min(0.65, 0.25 + ((noise - 0.50) * 10.0)); } 
    }, 
    structures: { 
      rate: 0.0, 
      pools: [{ chance: 1.00, id: 'ABANDONED_VILLAGE' }] 
  ...
  HOUSE_INTERIOR: { 
    ...
    allowedWeather: [], 
    music: { day: 'houseInteriorBgmDay', night: 'houseInteriorBgmNight', battle: 'houseInteriorBattleBgm' }, 
