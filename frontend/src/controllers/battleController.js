...
            fled: false // --- NEW: Track if the party successfully escaped
        };
...
        const maxInsight = detailedStats.maxInsight?.total || 10;
        ...
        // --- UPDATED: Pass teamAllegiance down to extraction ---
...
    }
    ...
    // --- UPDATED: Receive teamAllegiance and inject 'retreat' ---
...
        });
        ...
        // NEW: Automatically grant the retreat ability if they are an ally
...
            
            // --- NEW: Handle Flee Outcome ---
            if (result.fled) {
                this.state.fled = true; 

                // Wipe any pending turns so combat stops immediately
                this.state.turnQueue = [
                    { type: TURN_TYPES.MESSAGE_STATUS, message: `${actor.name}'s party escapes!` },
                    { type: TURN_TYPES.BATTLE_END }
                ];
                break; // Break the target loop immediately
            }
            // --------------------------------
...
            // 1. Save vital stats back to the original entity
...
            
            // 2. Safely remove non-persistent status effects using the model's method
...
        // Let the overworld manager know the outcome
