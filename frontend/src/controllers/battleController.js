...
    handleActionSelection(key) {
        ...
        if (key === 'ArrowRight') {
            ...
            if (this.state.selectedAction.targeting && this.state.selectedAction.targeting.scope === 'self') {
                ...
                // NEW: Determine if this action targets allies or enemies
                const isFriendlyTarget = this.state.selectedAction.targeting && this.state.selectedAction.targeting.scope === 'ally';
                
                // Save the group so handleTargetSelection knows which array to use
                this.state.targetGroup = isFriendlyTarget ? 'party' : 'enemy';
                
                // Grab the correct array based on the scope
                const targetArray = isFriendlyTarget ? this.state.activeParty : this.state.activeEnemies;
                
                // Find the first valid, living target in that specific array
...
    handleTargetSelection(key) {
        // NEW: Figure out which array we are cycling through based on the stored group
        ...
        const cycleTarget = (direction) => {
            ...
            for (let i = 0; i < targetCount; i++) {
                ...
                // Works perfectly for both party members and enemies!
        ...
        if (key === 'ArrowDown' || key === 'ArrowRight') {
            ...
            this.state.targetGroup = null; // Clean up the state
        }
    }
...
        } else {
            // ---> NEW: Apply end of round recovery right before handing control back to the player
...
            ...this.state.activeParty.filter(p => !p.isDead()),
            ...this.state.activeEnemies.filter(e => !e.isDead())
        ];

        activeCombatants.forEach(combatant => {
            // Using the exact variable names exported by StatCalculator
