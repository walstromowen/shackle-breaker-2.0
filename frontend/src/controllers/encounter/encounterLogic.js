...
export class EncounterLogic { 
    static checkConditions(decision, context = {}) { 
        ...
        return decision.conditions.every(cond => { 
            switch (cond.type) { 
                case "has_other_party_members": return gameState.party?.members?.length > 1; 
                case "has_item": return InventorySystem.hasItem(cond.itemId, cond.qty || 1); 
                case "context_flag_set": return !!context[cond.flagId]; 
                case "context_flag_not_set": return !context[cond.flagId]; 
                default: return true; 
            } 
        }); 
    } 
    ...
    static resolveResults(resultsArray, model, worldManager) { 
        const response = { 
            messages: [], shouldEndEncounter: false, endEncounterPayload: null, 
            modelChanged: false, newModel: null, stageChanged: false, newStageId: null, 
            isGameOver: false, forceCharacterSwitch: false 
        }; 
            ...
            switch (type) { 
                ...
                case "ROLL_LOOT_TABLE": 
                    if (payload.lootTableId) { 
                        ...
                        const lootModel = LootTableFactory.generateLoot(payload.lootTableId, computedRolls, payload.baseCurrency || 0); 
