...
export class EncounterController extends BaseController { 
    constructor(input, config, worldManager) { 
        ...
        this.rollData = { displayVal: "?", d20: 0, mod: 0, total: 0, dc: 0, isSuccess: false, duration: 1.5 }; 
        this.lastText = ""; 
        this.textTimer = 0; 
        this.skipMessageAnimation = false; 
        
        // Cooldown to prevent spam clicking (Standardized to 150ms for snappy feel) 
    ...
    update(dt) { 
        ...
        if (this.actionPhase === 'message') { 
            ...
            if (this.skipMessageAnimation || this.textTimer >= (totalTypingTime + padding)) { 
                this.skipMessageAnimation = false; 
            ...
            this.rollTimer -= dt; 
            let progress = Math.min(Math.max(1.0 - (this.rollTimer / 0.8), 0), 1); 
    ...
    onClick(hitboxId) { 
        ...
        const padding = Math.max(0.8, Math.min(2.5, this.lastText.length * 0.02)); 
        const isAnimatingText = this.textTimer < (totalTypingTime + padding); 
        ...
        if (skipPhases.includes(this.actionPhase) || isAnimatingText) { 
            ...
            this.textTimer = totalTypingTime + padding; 
            this.inputCooldown = 0.15; 
            return; 
        } 
    ...
    applyLogicResults(resultsArray) { 
        ...
        if (isTotalWipe) { 
            this._triggerImageTransition(2.0); // Longer transition for a total wipe 
