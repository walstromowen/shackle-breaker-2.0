...
this.dropdownHitboxes = [
  ...
  onClick(hitboxId) {
    ...
    switch (hitboxId) {
        ...
        // --- NEW: Crafting Button Click Logic ---
        case 'btn_crafting':
  ...
  // --- Inside OverworldController.js -> handleKeyDown ---
...
handleKeyDown(code, e) {
    ...
    if (obj && obj.interaction) {
      ...
      if (obj.interaction.type === 'CRAFTING_STATION' || obj.interaction.type === 'WORKSHOP') {
          events.emit('CHANGE_SCENE', { 
              scene: 'workshop', 
              data: { 
                  station: obj.interaction.station || 'any', 
                  title: obj.interaction.title || obj.name || 'Workshop' 
              } 
          });
          return;
      }
  ...
  createPlayerEntity() {
    ...
    return {
      ...
      animFrame: 0,
      animTimer: 0,
      light: { hasLight: true, radius: 4, color: '255, 200, 100', maxAlpha: 0.5, flickerAmp: 0.1 }
    };
  }
}