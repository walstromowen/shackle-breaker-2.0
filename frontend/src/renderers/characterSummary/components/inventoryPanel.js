...
export class InventoryPanel { 
        ...
        if (!inventory || inventory.length === 0) { 
            ...
            inventory.forEach((item, index) => { 
                ...
                if (itemY + this.SLOT_SIZE >= listY && itemY <= listY + listH) { 
                    hitboxes.push({ id: `INV_ITEM_${index}`, x: itemX, y: itemY, w: this.SLOT_SIZE, h: this.SLOT_SIZE, type: 'inventory', index: index }); 
                } 
    ...
    drawScrollBar(x, y, viewportH, contentH, scrollOffset, hitboxes) { 
        ...
        if (hitboxes) { 
            hitboxes.push({ id: 'INV_SCROLLBAR_THUMB', type: 'ui', x: x - 10, y: thumbY, w: this.SCROLLBAR_WIDTH + 19, h: thumbH }); 
        } 
    } 
