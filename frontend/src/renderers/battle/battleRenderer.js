...
export class BattleRenderer {
  ...
  render(state) {
    ...
    // =========================================================================
    // 1. Draw Background (Static - Camera Shake Removed)
    // =========================================================================
    ...
    // Hardcode offsets to 0 so the background never moves
    const bgOffsetX = 0;
    const bgOffsetY = 0;

    // Render the baseline background texture layer
    if (baseBgImg) {
    ...
    // Render the active overlaid movement animation background sequence
    if (anim && typeof anim.getActiveBackground === 'function') {
      ...
      if (activeBg) {
        ...
        // TRAP FIX: Safely translate the 'current' keyword to fall back cleanly to base arena maps
