...
export class EncounterRenderer {
    ...
    render(ctx, state) {
        ...
        const { imageInfo, transition, text, title, stageName, encounter, decisions, rewards, ui: uiState, party = [], currency = 0, skipMessageAnimation, textTimer, actionPhase, rollData, hoveredElement, scrollOffsets, onLayoutUpdate } = state;
        ...
        party.slice(0, 3).forEach((member, index) => {
            ...
            // Only pass the transition for the active character (index 0)
            this.drawPartyMember(ctx, ui, member, 0, currentY, leftW, nameY, UITheme.fonts.body, targetAreaSize, index === 0 ? transition : null);
        ...
        this.drawCenteredWrappedText(ctx, ui, encounterTitle, centerX + (centerW / 2), 96, centerW - 96, 67, UITheme.fonts.header, UITheme.colors.textMain, true);
        ...
        scrollBounds.text = { bounds: { x: centerX + 64, y: textViewportY, w: centerW - 128, h: textViewportH }, maxScroll: textMaxScroll, viewportH: textViewportH };
        ui.startClip(centerX + 64, textViewportY, centerW - 128, textViewportH);
        ui.drawWrappedText(visibleText, centerX + 96, textViewportY - textScrollOffset, centerW - 192, textLineHeight, UITheme.fonts.body, UITheme.colors.textMain);
        ui.endClip();
        ...
        if (showDecisions) {
            ...
            if (rewards) {
                ...
                scrollBounds.decisions = { bounds: { x: btnX - 48, y: decisionViewportY, w: btnW + 96, h: decisionViewportH }, maxScroll: maxScroll, viewportH: decisionViewportH };
                ...
                decisions.forEach((opt, index) => {
                    ...
                    if (renderY + blockHeight > decisionViewportY && renderY < decisionViewportY + decisionViewportH) {
                        this.hotspots.push({ id: hitId, x: btnX - 36, y: renderY - 8, w: btnW + 72, h: decisionHeight + 16, hoverSfx: 'hoverTick', clickSfx: 'hoverTick' });
                    }
    ...
    drawPartyMember(ctx, ui, member, x, y, colWidth, nameY, font, targetAreaSize, transition = null) {
        const nativeRes = 128;
        const imgScale = 2;
        const drawSize = nativeRes * imgScale;
        ...
        // Trigger a change if the name is different, OR if the same character went from Alive to Dead
        ...
        if (transActive && characterChanged) {
            ...
            ctx.save();
            ctx.filter = `brightness(${100 - p * 100}%) grayscale(${p * 100}%)`;
            drawCharSprite(prevMember, 1.0 - p);
            if (prevMember.hp <= 0) {
                ctx.fillStyle = `rgba(100, 0, 0, ${(1.0 - p) * 0.6})`;
                ctx.fillRect(pX, pY, targetAreaSize, targetAreaSize);
            }
            ctx.restore();
            ...
            if (member.hp <= 0 && p > 0.5) {
                ctx.fillStyle = `rgba(100, 0, 0, ${(p - 0.5) * 1.2})`; 
                ctx.fillRect(pX, pY, targetAreaSize, targetAreaSize);
            }
        } else {
        ...
        const displayedMember = (characterChanged && transition.progress < 0.5) ? prevMember : member;
        
        // 🌟 FADE OUT THE STATS DURING THE TRANSITION 🌟
        let statAlpha = 1.0;
        if (transActive && characterChanged) {
            // Dips to 0 at exactly 50% progress, then fades back in
            statAlpha = Math.abs(transition.progress - 0.5) * 2;
        }

        ctx.save();
        ctx.globalAlpha = statAlpha;
        
        this.drawStatusEffects(ctx, ui, displayedMember, pX, pY, targetAreaSize);
        ...
        // Fallbacks added here to ensure NaN is never processed visually
        const hp = displayedMember.hp || 0;
        const maxHp = displayedMember.maxHp || 1;
        const stm = displayedMember.stamina || 0;
        const maxStm = displayedMember.maxStamina || 1;
        const ins = displayedMember.insight || 0;
        const maxIns = displayedMember.maxInsight || 1;

        this.drawStatRow(ui, "HP", hp, maxHp, startX, statY, barW, 14, numW, labelW, UITheme.colors.hp, UITheme.colors.hpDim);
        statY += 58;
        this.drawStatRow(ui, "STM", stm, maxStm, startX, statY, barW, 14, numW, labelW, UITheme.colors.stm, UITheme.colors.stmDim);
        statY += 58;
        this.drawStatRow(ui, "INS", ins, maxIns, startX, statY, barW, 14, numW, labelW, UITheme.colors.ins, UITheme.colors.insDim);
        
        ctx.restore(); // Restores shadow Blur and Alpha back to defaults
    }
    ...
    drawStatusEffects(ctx, ui, member, pX, pY, targetAreaSize) {
        ...
        member.statusEffects.forEach(effect => {
            if (effect.icon) {
                ctx.drawImage(sheet, effect.icon.col * srcSize, effect.icon.row * srcSize, srcSize, srcSize, drawX, drawY, drawSize, drawSize);
