...
export class EncounterRenderer {
  ...
  render(ctx, state) {
    ...
    const {
      ...
      ui: uiState, party = [], currency = 0, skipMessageAnimation, textTimer,
      actionPhase, rollData, hoveredElement, scrollOffsets, onLayoutUpdate
    } = state;
      ...
      if (rewards) {
        ...
        decisions.forEach((opt, index) => {
          ...
          if (renderY + blockHeight > decisionViewportY && renderY < decisionViewportY + decisionViewportH) {
            this.hotspots.push({
              id: hitId,
              x: btnX - 36, y: renderY - 8, w: btnW + 72, h: decisionHeight + 16,
