...
export class OverworldUIRenderer {
    ...
    render(ctx, state) {
        // --- NEW: Added facedObject and camera to destructuring ---
        const { hoveredHitboxId, isMenuOpen, menuToggleHitbox, dropdownHitboxes, trackedQuests, toasts, facedObject, camera } = state || {};
        ...
        // --- NEW: Draw Interactable Object Brackets ---
        if (facedObject && camera) {
            this.drawInteractionBrackets(ctx, ui, facedObject, camera);
            this.drawInteractionTooltip(ctx, ui, ctx.canvas, state.facedObject);
        }
    ...
    // --- NEW: Method to render animated in/out brackets (Scaled to Map) ---
    drawInteractionBrackets(ctx, ui, facedObject, camera) {
        ...
        // 2. Replicate MapRenderer's camera offset logic
        ...
        // 4. Translate world position to screen position, applying the scale
        const screenX = Math.floor((col * TILE_SIZE - renderCamX) * scale);
        const screenY = Math.floor((row * TILE_SIZE - renderCamY) * scale);

        // 5. Account for MapRenderer's object sizing and vertical offset for tall objects
        ...
        // 6. --- NEW: "In and Out" Animation Logic ---
        const now = performance.now();
        const pulseSpeed = 350; // Controls how fast the brackets breathe
        
        // Sine wave between 0.0 and 1.0
        const wave = (Math.sin(now / pulseSpeed) + 1) / 2; 
        
        // Max distance the brackets move outward (e.g., 4 unscaled pixels)
        const baseMaxPadding = 4; 
        
        // Multiply by 'scale' so the movement distance matches the zoomed map
        const padding = wave * baseMaxPadding * scale; 
        
        // Alpha pulses between 0.5 (expanded) and 1.0 (snug)
        const alpha = 0.5 + ((1 - wave) * 0.5); 

        ctx.save();
        ctx.globalAlpha = alpha;

        // Draw the brackets, expanding outward by 'padding' in all directions
        ui.drawSelectionBrackets(
            screenX - padding,
            drawY - padding,
            drawW + (padding * 2),
            drawH + (padding * 2),
            12, // Corner line length
            UITheme.colors.borderHighlight || "#FFD700"
        );

        ctx.restore();
    }
    // --- NEW: Method to draw bottom-right key prompts ---
    // --- UPDATED: Method to draw bottom-right key prompts matching UI theme (No Brackets) ---
    drawInteractionTooltip(ctx, ui, canvas, facedObject) {
        if (!facedObject) return;

        const text = "Interact";
        const shortcut = "I";
        const margin = 24; // Distance from the screen edge
        const height = 44; // Match standard toast/button height
        ...
        // 1. Measure text width using your theme's body font
        ctx.font = UITheme.fonts.body || "16px sans-serif";
        const textWidth = ctx.measureText(text).width;

        // 2. Calculate panel dimensions (padding + badge + gap + text + padding)
        const gap = 10;
        ...
        // 3. Draw Active Panel Background
        const bgColor = "rgba(65, 58, 50, 0.8)"; // Matches active square button color
        ui.drawPanel(startX, startY, width, height, bgColor);

        // 4. Draw Shortcut Badge (the little dark square for the "I" key)
    ...
    drawToasts(ctx, ui, toasts) {
        ...
        const baseY = ctx.canvas.height - 150; // Start near bottom right
        ...
        for (let i = toasts.length - 1; i >= 0; i--) {
            ...
            if (iconSheet) {
                ...
                ctx.drawImage(
                    ...
                    startX + 12, floatY + (toastHeight / 2) - (dstSize / 2), dstSize, dstSize
                );
            }

            // --- DRAW TEXT TIGHT TO THE ICON ---
            ui.drawText(
                textString,
                startX + 50, // (startX + 12 padding + 28 icon + 10 gap)
    ...
    drawQuestTracker(ctx, ui, quests) {
        // ... (Keep your existing drawQuestTracker code)
        const padding = 24;
        ...
        for (const quest of quests) {
            ui.drawText(
                ...
                startX + 16, currentY + (titleHeight / 2),
                UITheme.fonts.body, UITheme.colors.states.hoverText || "#FFD700", "left", "middle"
            );
            ...
            if (quest.objectives) {
                for (const obj of quest.objectives) {
                    ...
                    ui.drawText(
                        `- ${obj.description}`,
                        startX + 24, currentY + (lineHeight / 2),
                        UITheme.fonts.cardSmall, textColor, "left", "middle"
                    );

                    ui.drawText(
                        progressText,
                        startX + trackerWidth - 16, currentY + (lineHeight / 2),
                        UITheme.fonts.cardSmall, textColor, "right", "middle"
                    );
                    currentY += lineHeight;
                }
            }
    ...
    drawSquareButton(ctx, ui, btn, isHovered, isActiveOverride = false) {
        // ... (Keep your existing drawSquareButton code)
        ...
        if (btn.shortcut) {
            ...
            ui.drawText(
                btn.shortcut,
                bX + (badgeSize / 2), bY + (badgeSize / 2),
                UITheme.fonts.cardSmall, UITheme.colors.textMuted, "center", "middle"
            );
        }
        ...
        if (uiSheet && btn.spriteCol !== undefined && btn.spriteRow !== undefined) {
            ...
            ctx.drawImage(
                uiSheet, srcX, srcY, iconSize, iconSize,
        ...
        ui.drawText(
            btn.label,
            centerX, textY,
            UITheme.fonts.cardSmall, textColor, "center", "middle"
        );
    }
}